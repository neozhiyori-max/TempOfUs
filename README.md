...
各役職の陣営、能力、ゲーム内ボタン、ペナルティ、勝利条件、実装状況、ホスト設定の操作方法は、**[役職ガイド（日本語）](docs/ROLE_GUIDE_JA.md)** を参照してください。会議中だけ使えるマッドゲッサーの `推測` 操作も、このガイドに記載しています。役職ごとの参考MOD、元役職、リポジトリURL、ライセンス、コード移植か仕様参照か、tempMOD側の差分は、**[役職移行・クレジット一覧](docs/ROLE_MIGRATION_PLAN_JA.md)** に公開します。

## 実装済みの主な機能

| 分類 | 内容 |
| --- | --- |
| 役職 | クルー10、インポスター26、第三陣営24。シェリフ、ドクター、アンダーテイカー、マッドゲッサー、ジェスター、アルソニスト、ラバーズなどを含みます。役職ごとの詳細は[日本語役職ガイド](docs/ROLE_GUIDE_JA.md)、参考MOD・ライセンス・差分は[役職移行・クレジット一覧](docs/ROLE_MIGRATION_PLAN_JA.md)を参照してください。 |
...
tempMODは [GPL-3.0](LICENSE) で公開します。役職を移植または仕様参照する場合は、役職単位で **参考MOD、元役職、URL、ライセンス、コード移植か仕様参照か、tempMOD側の差分** を[役職移行・クレジット一覧](docs/ROLE_MIGRATION_PLAN_JA.md)に記載します。開始演出などの構造設計では [SuperNewRoles](https://github.com/SuperNewRoles/SuperNewRoles) を参考にし、当リポジトリで独自実装しています。詳細な通知は [NOTICE.md](NOTICE.md) を参照してください。
