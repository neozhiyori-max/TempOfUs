...
| 第三陣営 | シュレディンガーの猫 | 第1段階移植・実機検証待ち | SNR `SchrodingersCat.cs`、`SchrodingersCatAbility.cs` | 手動同調ボタンを廃止。最初のインポスター／ジャッカル／クルーからの攻撃を無効化し、攻撃者の陣営へ自動同調する。SNR既定の同調後キルCD 45秒を適用する。 |
