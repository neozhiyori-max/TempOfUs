...
namespace TownOfUs.CrewmateRoles.SwapperMod
{
    public class ShowHideButtonsSwapper
    {
        ...
        public static Dictionary<byte, int> CalculateVotesSwap(MeetingHud __instance)
        {
            ...
            {
                if (player.PlayerId == SwapVotes.Swap1.PlayerId) swapPlayer1 = player;
                if (player.PlayerId == SwapVotes.Swap2.PlayerId) swapPlayer2 = player;
            }
            ...
            var swap1 = 0;
            if (self.TryGetValue(SwapVotes.Swap1.PlayerId, out var value)) swap1 = value;

            var swap2 = 0;
            if (self.TryGetValue(SwapVotes.Swap2.PlayerId, out var value2)) swap2 = value2;

            self[SwapVotes.Swap2.PlayerId] = swap1;
            self[SwapVotes.Swap1.PlayerId] = swap2;
        ...
        public static class Confirm
        {
            public static bool Prefix(MeetingHud __instance)
            {
                ...
                Utils.Rpc(CustomRPC.SetSwaps, SwapVotes.Swap1.PlayerId, SwapVotes.Swap2.PlayerId);
                return true;
            }
        }
        ...
        public static class CheckForEndVoting
        {
            private static bool CheckVoted(PlayerVoteArea playerVoteArea)
            {
                ...
                var playerInfo = GameData.Instance.GetPlayerById(playerVoteArea.PlayerId);
                if (playerInfo == null)
                    return true;
