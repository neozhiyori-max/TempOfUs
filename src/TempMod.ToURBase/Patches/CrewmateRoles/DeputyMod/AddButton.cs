...
namespace TownOfUs.CrewmateRoles.DeputyMod
{
    public class AddButtonDeputy
    {
        ...
        private static bool IsExempt(PlayerVoteArea voteArea)
        {
            ...
            var player = Utils.PlayerById(voteArea.PlayerId);
        ...
        {
            var targetId = voteArea.PlayerId;
        ...
        private static Action Shoot(Deputy role, PlayerVoteArea voteArea)
        {
            void Listener()
            {
                var target = Utils.PlayerById(voteArea.PlayerId);
        ...
        public static void Shoot(Deputy deputy, PlayerControl player, bool checkLover = true)
        {
            ...
            foreach (var role in blackmailers)
            {
                if (role.Blackmailed != null && !blackmailed.Contains(role.Blackmailed))
                {
                    ...
                    if (voteArea.PlayerId == role.Blackmailed.PlayerId)
                    {
            ...
            if (PlayerControl.LocalPlayer.Is(RoleEnum.Vigilante) && !PlayerControl.LocalPlayer.Data.IsDead)
            {
                ...
                ShowHideButtonsVigi.HideTarget(vigi, voteArea.PlayerId);
            }

            if (PlayerControl.LocalPlayer.Is(AbilityEnum.Assassin) && !PlayerControl.LocalPlayer.Data.IsDead)
            {
                var assassin = Ability.GetAbility<Assassin>(PlayerControl.LocalPlayer);
                ShowHideButtons.HideTarget(assassin, voteArea.PlayerId);
            }
            ...
            if (PlayerControl.LocalPlayer.Is(RoleEnum.Doomsayer) && !PlayerControl.LocalPlayer.Data.IsDead)
            {
                ...
                ShowHideButtonsDoom.HideTarget(doom, voteArea.PlayerId);
            }

            if (PlayerControl.LocalPlayer.Is(RoleEnum.Deputy) && !PlayerControl.LocalPlayer.Data.IsDead)
            {
                var dep = Role.GetRole<Deputy>(PlayerControl.LocalPlayer);
                if (dep.Buttons.Count > 0 && dep.Buttons[voteArea.PlayerId] != null)
                {
                    dep.Buttons[voteArea.PlayerId].SetActive(false);
                    dep.Buttons[voteArea.PlayerId].GetComponent<PassiveButton>().OnClick = new Button.ButtonClickedEvent();
                }
            }
            ...
            if (PlayerControl.LocalPlayer.Is(RoleEnum.Swapper) && !PlayerControl.LocalPlayer.Data.IsDead)
            {
                ...
                {
                    if (swapper.ListOfActives[i].Item1 == voteArea.PlayerId)
                    {
                        index = i;
                        break;
                    }
            ...
            {
                if (playerVoteArea.VotedForId != player.PlayerId) continue;
                playerVoteArea.UnsetVote();
                var voteAreaPlayer = Utils.PlayerById(playerVoteArea.PlayerId);
            ...
            if (PlayerControl.LocalPlayer.Is(RoleEnum.Imitator) && !PlayerControl.LocalPlayer.Data.IsDead)
            {
                ...
                if (MeetingHud.Instance.state != MeetingHud.MeetingStates.Results && MeetingHud.Instance.state != MeetingHud.MeetingStates.Proceeding)
                {
