    ...
    public class AddRevealButton
    {
        ...
        public static void RemoveAssassin(Mayor mayor)
        {
            PlayerVoteArea voteArea = MeetingHud.Instance.playerStates.First(
                x => x.PlayerId == mayor.Player.PlayerId);
            ...
            else if (PlayerControl.LocalPlayer.Is(RoleEnum.Doomsayer))
            {
                ...
                ShowHideButtonsDoom.HideTarget(doomsayer, voteArea.PlayerId);
                voteArea.NameText.transform.localPosition = new Vector3(0.3384f, 0.0311f, -0.1f);
                foreach (var (targetId, guessText) in doomsayer.RoleGuess)
                {
                    if (!guessText.isActiveAndEnabled || voteArea.PlayerId != targetId) continue;
            ...
            else if (PlayerControl.LocalPlayer.Is(RoleEnum.Vigilante))
            {
                ...
                ShowHideButtonsVigi.HideTarget(vigilante, voteArea.PlayerId);
                voteArea.NameText.transform.localPosition = new Vector3(0.3384f, 0.0311f, -0.1f);
            }
        }
        ...
        public static void AddMayorButtons(MeetingHud __instance)
        {
            ...
            for (var i = 0; i < __instance.playerStates.Length; i++)
                if (PlayerControl.LocalPlayer.PlayerId == __instance.playerStates[i].PlayerId)
                {
