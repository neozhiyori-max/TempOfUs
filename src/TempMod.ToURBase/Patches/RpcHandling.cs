            ...
            public static void Postfix([HarmonyArgument(0)] byte callId, [HarmonyArgument(1)] MessageReader reader)
            {
                ...
                switch ((CustomRPC) callId)
                {
                    ...
                    case CustomRPC.SetSwaps:
                        ...
                        SwapVotes.Swap1 =
                            MeetingHud.Instance.playerStates.FirstOrDefault(x => x.PlayerId == readSByte);
                        readSByte2 = reader.ReadSByte();
                        SwapVotes.Swap2 =
                            MeetingHud.Instance.playerStates.FirstOrDefault(x => x.PlayerId == readSByte2);
                                        ...
                                        try
                                        {
                                            foreach (var role in Role.GetRoles(RoleEnum.Blackmailer))
                                            {
                                                foreach (var voteArea in MeetingHud.Instance.playerStates)
                                                {
                                                    ...
                                                    if (voteArea.PlayerId == bmer.Blackmailed.PlayerId)
                                                    {
