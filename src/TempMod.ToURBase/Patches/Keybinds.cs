...
namespace TownOfUs
{
    ...
    public sealed class AssassinVigilanteKeybinds
    {
        ...
        public static void Postfix(MeetingHud __instance)
        {
            ...
            if (__instance.state == MeetingHud.MeetingStates.Discussion) return;
            var role = Role.GetRole(PlayerControl.LocalPlayer);

            foreach (var player in __instance.playerStates)
            {
                if (!HighlightedPlayer) break;
                if (player.PlayerId == HighlightedPlayer.PlayerId)
                {
            ...
            if (role is Vigilante || role.Player.Is(AbilityEnum.Assassin) || role.Player.Is(RoleEnum.Doomsayer))
            {
                ...
                var players = __instance.playerStates.Where(x => (guesser as IGuesser).Buttons[x.PlayerId] != (null, null, null, null)
                                                                  && x.PlayerId != role.Player.PlayerId).ToList();
                ...
                {
                    (guesser as IGuesser).Buttons[HighlightedPlayer.PlayerId].Item2.GetComponent<PassiveButton>().OnClick.Invoke();
                }
                else if (ReInput.players.GetPlayer(0).GetButtonDown("ToU cycle -"))
                {
                    (guesser as IGuesser).Buttons[HighlightedPlayer.PlayerId].Item1.GetComponent<PassiveButton>().OnClick.Invoke();
                }
                else if (ReInput.players.GetPlayer(0).GetButtonDown("ToU confirm"))
                {
                    (guesser as IGuesser).Buttons[HighlightedPlayer.PlayerId].Item3.GetComponent<PassiveButton>().OnClick.Invoke();
                }
            }
        }
